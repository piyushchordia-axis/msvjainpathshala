...
- Do not hardcode domain URLs or hostnames in frontend code. Use `process.env.EXPO_PUBLIC_API_URL` for API base URL configuration, defaulting to `http://localhost:5000` in development. Use `getApiUrl()` from `@/lib/query-client` for API requests.
...
- The Expo App runs on port 8081. Access at `http://localhost:8081`.
- The Express backend runs on port 5000. It serves APIs for the app.
- There are two processes to manage:
  - **Backend**: Start with `node server/index.js > /tmp/backend.log 2>&1 &`. Restart after any server/backend changes by killing and restarting the process. Do NOT restart if you only made frontend changes.
  - **Frontend**: Start with `npx expo start --port 8081 > /tmp/expo.log 2>&1 &`. Since the Expo dev server has Hot Module Reloading, it automatically refreshes the app after most code changes. Only restart when you update dependencies or fix a startup error.
- To check process status: `lsof -i :8081` and `lsof -i :5000`
- To restart backend: `kill $(lsof -t -i:5000) 2>/dev/null; sleep 1; node server/index.js > /tmp/backend.log 2>&1 &`
- Scan the QR code from `http://localhost:8081` to test on a physical device via Expo Go.
- When the app is ready to deploy, refer to the deployment skill for publishing options (EAS Build for app stores, or web hosting for the web version).
...
- Scan the QR code shown in the Expo terminal (`npx expo start`) to test on a physical device via Expo Go
- Hot module reloading (HMR) is enabled — no need to restart the dev server for most code changes
- Use `npx expo start --web` to also run the web version at `http://localhost:8081`

## Forbidden Changes
- NEVER edit package.json directly. Use npm/yarn to install packages (see package-management skill).
...
- When running Expo locally, use `npx expo start` via the Bash tool. Set PORT and other env vars explicitly if needed: `PORT=8081 npx expo start`.
