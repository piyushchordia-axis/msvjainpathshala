---
name: mockup-extract
description: "Use when the user wants to pull an existing component from their main app into the mockup sandbox — whether to redesign it, create variants, or simply preview it in isolation. Also activate when the user asks to redesign, improve, or create variants of any UI that already exists as code in the main app, even if they don't explicitly say 'extract' — the real source code must be used as the starting point, never hand-coded approximations. Copies the component and its dependencies into the mockup sandbox, rewrites imports, and stubs external dependencies. Activate when the user says 'show my current X', 'redesign my existing Y', 'create variants of my current Z', 'improve my Y', or wants to iterate on something already in the app."
---

# Mockup Extract Skill

Pull an existing component from the main app into the `artifacts/mockup-sandbox/` folder so it can be previewed in the browser and used as a starting point for design iteration.
...
The mockup sandbox must be set up first. If `artifacts/mockup-sandbox/` doesn't exist, activate the `mockup-sandbox` skill to set it up before proceeding.

## Do Not Preview the Main App Directly

When the user asks to "show my component" or "redesign my navbar," do **not** point a browser at the main app's dev server URL. This is the wrong approach because:
...
- Editing the main app code changes the preview live, making it impossible to compare before/after

Instead, always extract the component into `artifacts/mockup-sandbox/` and preview at `http://localhost:5173/preview/{group}/{ComponentName}`.
...
Do **not** edit the sandbox's global `index.css` — that leaks this app's tokens into every unrelated mockup group. Do **not** add font `<link>` tags to `artifacts/mockup-sandbox/index.html` — that file is shared across all mockup groups. Put any additional app-specific fonts in `_group.css` via `@import` to keep them scoped to this extraction.
...
### Step 6: Type-check, preview, and create variants
...
Preview in browser: `http://localhost:5173/preview/{group}/Current`

Use `mcp__Claude_in_Chrome__navigate` to open it and `mcp__Claude_in_Chrome__computer` with `action: "screenshot"` to capture and show the user. Label it "Current", then create new variant files alongside it for comparison. See the `mockup-sandbox` skill for the full preview and comparison workflow.
