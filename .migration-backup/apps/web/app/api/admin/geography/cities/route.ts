/**
 * POST /api/admin/geography/cities — proxy for POST /v1/geography/cities
 * (state_admin+).
 */
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const res = await client.post('/v1/geography/cities', parsed);
    ...
    return NextResponse.json(
      { error: { code: 'ERR_INTERNAL', message: 'Could not create city' } },
      { status: 500 },
    );
  }
}
