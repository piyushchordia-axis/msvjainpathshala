...
export function Icon({ name, size = 22, color = JPColors.textPrimary, fill = 'none' }: IconProps) {
  ...
  const paths: Record<IconName, React.ReactNode> = {
    ...
    check: (
      <Path
        d="M9 11l3 3 4-4M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2Z"
        {...P}
      />
    ),
    bell: (
    ...
    sparkles: (
      <G {...P}>
        ...
        <Path d="m6 6 2 2M16 16l2 2M16 8l2-2M6 18l2-2" />
      </G>
    ),
  };
...
// `any` here matches the upstream design-system component pack's signature —
// keeping it lets us copy future updates verbatim.
...
export function PrimaryButton({
  ...
  return (
    ...
    >
      <Animated.View
        style={[styles.btnPrimary, disabled && styles.btnPrimaryDisabledShadow, animated, style]}
      >
        {loading ? <ActivityIndicator color="#FFFFFF" size="small" /> : (leftIcon ?? null)}
        {wrapTextChild(children, [styles.btnPrimaryLabel, disabled && { color: JPColors.textDim }])}
      </Animated.View>
    </Pressable>
  );
}
...
export function Avatar({ initials = 'A', size = 40, msv = false, gradient = true }: AvatarProps) {
  const radius = size / 2;
  const fontSize = size * 0.38;

  // Upstream pack builds a non-gradient fallback here but the JSX below
  // never references it. Keep as `_base` (lint exempts underscore-prefix)
  // until we drop the dead branch upstream.
  const _base = (
    <View
      style={[
        styles.avatarBase,
  ...
  return (
    <View style={{ width: size, height: size, position: 'relative' }}>
      {gradient ? (
        ...
        >
          <Text style={[styles.avatarInitials, { color: '#FFFFFF', fontSize }]}>{initials}</Text>
        </LinearGradient>
      ) : (
        <View
          style={[
        ...
        >
          <Text style={[styles.avatarInitials, { color: JPColors.maroon, fontSize }]}>
            {initials}
          </Text>
        </View>
      )}
      ...
      {msv ? (
        ...
        >
          <Text style={[styles.msvLabel, { fontSize: Math.max(8, size * 0.16) }]}>MSV</Text>
        </LinearGradient>
      ) : null}
    </View>
  );
...
const ATTENDANCE: Record<AttendanceStatus, { bg: string; fg: string; icon: IconName }> = {
...
export function Card({ children, elevated, style }: CardProps) {
  return (
    <View style={[styles.cardBase, elevated ? styles.cardElevated : styles.cardFlat, style]}>
      {children}
    </View>
  );
}
      ...
      {children.map((c) => {
        ...
        return (
          <Pressable
            ...
            style={[styles.childPill, on ? styles.childPillActive : styles.childPillInactive]}
          >
            <Avatar initials={c.initials} size={22} gradient={on} />
            <Text
              style={[styles.childLabel, { color: on ? JPColors.textPrimary : JPColors.textSub }]}
            >
              {c.name}
            </Text>
    ...
    <View style={styles.empty}>
      <Svg width={80} height={80} viewBox="0 0 64 64">
        <Ellipse
          ...
          rx="20"
          ry="6"
          stroke={JPColors.saffron300}
          strokeWidth={1.6}
          fill="none"
        />
        <Path
          d="M32 40c0-12-8-20-12-22 4 4 6 14 12 22Z"
          stroke={JPColors.saffron300}
          strokeWidth={1.6}
          fill="none"
        />
        ...
        />
        <Path
          d="M32 40c0-16-4-22-4-22 0 6 2 16 4 22Z"
          stroke={JPColors.saffron300}
          strokeWidth={1.6}
          fill="none"
        />
