/**
 * RN-shaped design tokens — copy of `jp-design-system/expo/tokens.ts` with
 * the nested shape (`tokens.color.brand.saffron`, `tokens.spacing[4]`,
 * `tokens.shadow[1]`) that the pre-built Expo components from
 * `jp-design-system/expo/components/*` and `jp-design-system/expo/mobile/components.tsx`
 * expect.
 *
 * Single source of truth still lives in `packages/design-tokens/tokens.json` —
 * any change there must be mirrored here. Two views over the same palette:
 *
 *   `@jp/design-tokens`       → flat (`JPColors.saffron`, `JPSpacing.sp4`)
 *   `@/theme/rn-tokens`       → nested (`tokens.color.brand.saffron`)
 *
 * Both must agree; if they drift, the parity check in `@jp/design-tokens`
 * fires on every boot.
 */
