...
#### `centres`
`id, city_id (fk), name, address_line, locality, pincode, lat numeric(10,7), lng numeric(10,7), gps_radius_m int default 150, contact_phone, contact_email, status (active|inactive), academic_year text, deleted_at`. Indexes: `(city_id, status)`, GIST/BTREE on `(lat, lng)`.
...
#### `sessions`
`id, batch_id, scheduled_date date, scheduled_start_time, scheduled_end_time, status (session_status_enum default 'scheduled'), shikshak_user_id (assigned), check_in_at timestamptz, check_in_lat numeric(10,7), check_in_lng numeric(10,7), check_in_distance_m int, check_out_at timestamptz, check_out_lat, check_out_lng, duration_minutes int, gps_haversine_m int, cancelled_at, cancellation_reason text, cancellation_by (fk→users)`. Indexes: `(batch_id, scheduled_date)`, `(shikshak_user_id, scheduled_date)`, `(scheduled_date, status)`.

#### `attendance`
`id, session_id (fk), student_id (fk), status (attendance_status_enum), marked_at, marked_by (fk→users), client_op_id (uuid, nullable, unique) for offline idempotency, notes text`. Unique `(session_id, student_id)`. Indexes: `(student_id, marked_at desc)`, `(session_id)`. Partial: `(student_id) where status='absent'` for streak/consecutive-absence checks.

#### `absence_notifications`
`id, student_id, parent_user_id, expected_session_id (nullable), expected_date date, reason text, created_at`. Index: `(student_id, expected_date)`.

#### `session_cancellations`
Embedded fields on `sessions` cover this; also write to `audit_logs`.

#### `centre_holidays`
`id, centre_id, name, start_date, end_date, created_by, notify_sent boolean default false`. Index: `(centre_id, start_date, end_date)`.

### 5.7 Punya

#### `punya_features`
Feature catalogue: `id, key (e.g. attendance_present, homework_approved, niyam_completion, manual_seva), default_points int, is_manual boolean, requires_reason boolean, scope (global|city), min_points, max_points`. Super-admin managed.
...
`id, user_id, client_op_id uuid unique, op_kind text, request_payload jsonb, response_payload jsonb, status sync_op_status_enum, error text, applied_at`. Unique `(user_id, client_op_id)`. TTL: rows older than 90 days purged by daily cron.
...
| POST | `/v1/sessions/:id/check-in` | shikshak | Body `{ lat, lng, accuracy_m, client_op_id }`. Computes distance from centre; flags if > centre.gps_radius_m. |
| POST | `/v1/sessions/:id/check-out` | shikshak | Body `{ lat, lng, client_op_id }`. |
| POST | `/v1/sessions/:id/attendance` | shikshak | Body `{ marks: [{ student_id, status, notes? }], client_op_id }`. Bulk. Idempotent. |
| PATCH | `/v1/sessions/:id/attendance/:student_id` | shikshak (same-day) | Edit single mark. |
| POST | `/v1/sessions/:id/cancel` | shikshak, sanchalak+, city_admin+ | Body `{ reason }`. Emits event → notifies parents. |
| GET | `/v1/students/:id/attendance` | parent (own), student-view, shikshak (own), sanchalak+ | Query `month=YYYY-MM`. |
...
| GET | `/v1/admin/centres/:id/holidays` | any |
...
| Method | Route | Roles |
|---|---|---|
| POST | `/v1/sync/batch` | jwt | Body `{ ops: [{ client_op_id, op_kind, payload }] }`. Returns per-op result. |
...
1. Shikshak opens "Today's Sessions" → taps Check-In on a scheduled session.
2. Client captures `lat, lng, accuracy_m`; rejects if `accuracy_m > 100`.
3. POST to `/v1/sessions/:id/check-in` with `client_op_id`.
4. Server:
   - Asserts session is `scheduled` or earlier check-in already exists (idempotent: same client_op_id returns same response).
   - Computes Haversine distance from centre's coords.
   - Sets `sessions.check_in_at`, `check_in_lat/lng`, `check_in_distance_m`, status=`in_progress`.
   - If distance > `centre.gps_radius_m`: still allows check-in but flags `session.flags = ['off_site_check_in']`; notifies sanchalak.
5. **Edge case — no-show flag:** scheduler job `shikshak-no-show-flag` runs hourly checking sessions scheduled to start ≥30min ago without check-in; flags + notifies sanchalak/city_admin.
6. **Edge case — double check-in:** if `check_in_at` already set with different `client_op_id`, return 409 `ALREADY_CHECKED_IN`.

### 8.3 Attendance Marking (Online + Offline)
1. Shikshak opens batch attendance screen.
2. Client either calls `/v1/batches/:id/students` (online) or reads cached roster from MMKV (offline).
3. Marks each student; **client_op_id** generated once per submission (UUID).
4. Online: POST `/v1/sessions/:id/attendance { marks, client_op_id }` immediately.
5. Offline: write to MMKV `pending_attendance` queue; sync engine picks up on reconnect.
6. **Idempotency:** server checks `sync_operations` for `client_op_id`. If exists with status=`success`, returns prior result.
7. Transactional logic:
   ```
   BEGIN;
     UPSERT attendance rows (unique on session_id, student_id);
     For each NEW present mark:
       INSERT punya_transactions(idempotency_key="attendance:{attendance_id}", ...);
       UPDATE punya_balances (atomic increment);
     Emit attendance.marked event;
   COMMIT;
   ```
8. **Edge case — late edit:** within same calendar day (centre timezone), shikshak can flip a mark. If switching from present→absent: punya reversal txn inserted (`reversal_of`).
9. **Edge case — student transferred mid-day:** rare; attendance still applies to original batch; manual reconciliation by sanchalak.
...
- Stored in `sync_operations` (for sync) or in Redis (`idem:{user_id}:{key}` TTL 24h) for general APIs.
- Repeating the same key returns the original response.

### 8.11 Conflict Resolution (Offline Sync)
- `sync_operations.client_op_id` is global per device → strict idempotency.
- For attendance: server's last write wins per `(session_id, student_id)`, but operations include their `marked_at`; if a stored mark has a newer `marked_at`, the offline op is recorded as `duplicate` and ignored.
- For Niyam submissions: each upload generates new `client_op_id`; no conflict possible (multiple submissions allowed).
...
- Scheduler runs daily at 23:00 IST.
- For each active student: counts attendance rows for last 3 scheduled-and-not-cancelled sessions in their batch.
- If all 3 = `absent` (and not pre-notified): flag → notify parent + city_admin.
...
- `/admin-dashboard` — receives `notice.broadcast`, live counts.
...
| Cron | Job |
|---|---|
| `0 6 * * *` IST | `birthday-wishes.job` |
| `0 1 1 * *` IST | `monthly-progress-report.job` |
| `30 0 1 * *` IST | `leaderboard-monthly-reset.job` |
| `0 23 * * *` IST | `consecutive-absence-flag.job` |
| `*/15 * * * *` | `shikshak-no-show-flag.job` |
| `0 2 * * *` IST | `analytics-aggregation.job` |
| `0 3 * * 0` IST | `punya-reconcile.job` (weekly) |
| `0 4 * * *` IST | `sync-operations-purge.job` (>90d) |
| `*/1 * * * *` | `leaderboard-refresh.job` |
| `0 5 * * *` IST | `niyam-streak-reset.job` |
...
- Idempotent operations on server via `client_op_id` (Section 8.10).
...
- **Warm path**: Postgres materialised views refreshed nightly (`mv_centre_engagement`, `mv_city_attendance_monthly`, `mv_donation_summary`, `mv_msv_funnel`).
...
**Build:**
- `sessions` module: `POST /v1/sessions/checkin` (shikshak GPS check-in), `POST /v1/sessions/checkout`. Haversine distance computed server-side; if check-in is >500m from `centres.gps_coordinates`, flag the session and notify sanchalak.
- `attendance` module: `POST /v1/attendance/mark` (bulk per-session, idempotent via `client_op_id`); enqueues `attendance.post_process` job that handles Punya award, streak update, parent notification, leaderboard refresh — all within a single Postgres transaction wrapped in a queue job for failure isolation.
- `absence_notifications`: `POST /v1/absences/notify` for parent advance-absence, surfaced in the shikshak's attendance marking screen.
- `session_cancellations`: `POST /v1/sessions/:id/cancel` with mandatory reason, fan-out to all batch parents.
- Shikshak attendance UI (mobile): batch roster screen, GPS check-in button, per-student Present/Absent/Late toggles, submit button. Full offline support — operations queue in MMKV `jp.queue.attendance` and replay on reconnect with idempotency keys.
- Parent attendance UI (mobile): calendar heatmap, multi-child selector, advance-absence form.
- Sanchalak/city_admin attendance dashboard (web): centre-level attendance %, shikshak check-in log with GPS coordinates and flag indicators.
- Consecutive-absence flag: scheduled job (`attendance.consecutive_check`) runs nightly; if a student missed 3 consecutive scheduled sessions, alert parent and city_admin.

**Exit criteria:** A shikshak can check in (with GPS verification), mark attendance for 20 students (including a few offline if network is killed mid-flow), check out, and the parent receives notifications for each child. Attendance percentage updates correctly accounting for holidays and cancelled sessions.
...
**Build:**
- `POST /v1/sync/batch` accepts an array of client-generated operations, each with `client_op_id`, `op_type`, `payload`, `client_timestamp`. Server processes idempotently using a `sync_operations` table with unique `(user_id, client_op_id)` index.
...
- Analytics module per Section 12 of this prompt: materialised views (`mv_centre_engagement`, `mv_punya_distribution`, `mv_msv_pipeline`, `mv_attendance_trends`, `mv_niyam_completion`); refreshed nightly via `analytics.refresh_views` cron.
