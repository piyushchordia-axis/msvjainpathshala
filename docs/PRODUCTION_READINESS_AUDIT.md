...
_Generated 2026-06-15 by a 50-agent audit: one deep auditor + one adversarial verifier per module, plus 6 cross-cutting sweeps. Every claim was checked against the **actual code** in `apps/*` and `lib/*` (at the time of the audit this source lived under `artifacts/*`, later renamed to `apps/*`). The pre-existing `docs/IMPLEMENTATION_STATUS.md` marks all 22 modules "✅" — **that document is materially inaccurate** and should not be trusted for launch decisions._
...
- **SMS OTP** — pluggable provider `apps/api-server/src/lib/sms.ts` (MSG91 + generic adapters, mock in dev/test), wired into `auth.ts`; prod fail-fasts without keys.
...
- **Mobile build** — `apps/jain-pathshala-mobile/eas.json` + iOS/Android bundle IDs.
...
6. **Uploads on ephemeral local disk.** `LocalDiskProvider` writes to `<cwd>/uploads` (gitignored, ephemeral). On autoscale/redeploy, **ID-card PNGs, 80G receipts, and progress-report PDFs vanish**, and aren't shared across instances. → `apps/api-server/src/lib/storage.ts:40-87`. *Fix: implement `S3StorageProvider` against the existing interface; select by env.*
...
10. **ID-card QR secret has no production guard** — falls back to the public dev value unlike `tokens.ts`/`file-tokens.ts` which throw. → `apps/api-server/src/lib/idcard-crypto.ts:8`. *Fix: throw in prod if `JP_AUTH_SECRET` is unset.*
11. **No DB-pool error listener and no process-level handlers.** `pool` has no `'error'` handler (idle-client errors on failover → unhandled `'error'` → crash); no `unhandledRejection`/`uncaughtException` handlers. → `lib/db/src/index.ts:13`, `apps/api-server/src/index.ts`. *Fix: add listeners + graceful shutdown.*
