...
Delegate using the Claude Code **Agent** tool. Your delegation message must include:
...
For parallel variants, launch all Agent calls in a single message with `run_in_background: true`:

```
Agent({
  description: "Pricing card variant — Bold",
  prompt: "Create artifacts/mockup-sandbox/src/components/mockups/pricing-cards/Bold.tsx\n\n## Design Brief\n[full brief from Step 4]\n\n## This Variant's Hypothesis\nName: Bold\nHypothesis: ...\n\nExport function name must match filename: export function Bold().\nUse Tailwind + shadcn/ui. Run npx tsc --noEmit when done.\nReport: file path created and any type errors.",
