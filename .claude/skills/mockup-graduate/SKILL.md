...
Move an approved mockup from `artifacts/mockup-sandbox/` into the main app. Transform the self-contained prototype into production code that matches the app's conventions.
...
If you need to parallelize graduation (e.g., graduate multiple pages at once), use a background Agent — graduation is an engineering task (understanding app architecture, transforming mockup code to production patterns, wiring routing and state) that requires codebase navigation, not creative visual output.

## Prerequisites

- The user must have identified which mockup to graduate (if multiple variants exist, ask which one)
- Locate the approved component at `artifacts/mockup-sandbox/src/components/mockups/{group}/{ComponentName}.tsx`

## Process

### Step 1: Identify the approved mockup

Confirm which mockup the user wants. Read the component file and extract key design decisions: colors, gradients, shadows, typography, layout approach, icons, and animations.
...
| Static images from `artifacts/mockup-sandbox/public/` | App's asset directory or CDN |

### Step 4: Install missing dependencies

Compare the mockup's imports against the main app's `package.json`. Install anything missing:
...
Add font links to `index.html` if needed.

### Step 5: Transform and place the component

Create the production component in the main app. Replace mock data with real data fetching, wire up navigation, connect to app state, and adapt UI components to the app's library. Copy any assets from `artifacts/mockup-sandbox/public/` to the main app's asset directory.

### Step 6: Update routing and verify

Add a route if the graduated component is a new page. If it replaces an existing component, update the import. Run the main app's type checker, restart the dev server, and confirm it renders correctly:

```bash
npx tsc --noEmit  # or: ruff check . / mypy .
```

### Step 7: Clean up (optional)

Ask the user if they want to remove the graduated mockup from the sandbox or keep it for reference. Don't clean up automatically.

## When Graduation Is Complex

Most graduations are straightforward — just proceed. Only pause and check with the user when the main app uses a fundamentally different design system than the mockup (requiring a full visual translation), or when the graduation would require a complete refactor of the existing backend.
