/**
 * Public homepage. Saffron hero block with the mandala motif on the
 * right, mission statement, three stat tiles.
 *
 * Copy is intentionally inlined here for Step 9 — once we have the
 * public-content step, this page reads from a CMS / i18n bundle.
 */
...
export default async function PublicHome({ params }: PageProps) {
  const { locale } = await params;
  return <PublicHomeContent isHindi={locale === 'hi'} />;
}

function PublicHomeContent({ isHindi }: { isHindi: boolean }) {
  // Lightly localised copy — full bilingual content lands with the
  // public-content step (CMS-driven).
  const copy = isHindi
    ? {
