...
export type HomeworkAdminPersona = "shikshak" | "sanchalak";
...
function CreateAssignmentModal({
  ...
  onClose: () => void;
  onCreated: () => void;
  /** When set (Sanchalak), only list batches at this centre. */
  ...
  // useAdminBatches: scopedBatchFilter is a no-op when scope.batchIds is null
  // (sanchalak) — centre membership only. Narrow further by the switcher.
  const items = useMemo(() => {
    const all = batches.data?.items ?? [];
    if (!centreId) return all;
    return all.filter((b) => b.centre_id === centreId);
  }, [batches.data?.items, centreId]);
          ...
          <View style={{ gap: 6 }}>
            ...
            {batches.isLoading ? (
              ...
              <Body muted>
                {centreId
                  ? hi
                    ? "इस केंद्र पर कोई बैच नहीं।"
                    : "No batches at this centre."
                  : hi
                    ? "कोई बैच नहीं मिला।"
                    : "No batches in your scope."}
              </Body>
            ) : (
              items.map((b) => {
...
/**
 * Shared Guruji / Sanchalak homework admin — assign, list, open submissions to grade.
 * Persona only changes empty-state copy and (for Sanchalak) the centre switcher.
 */
export default function HomeworkAdmin({ persona }: { persona: HomeworkAdminPersona }) {
  const c = useColors();
  const { hi } = useLocale();
  ...
  const items = useMemo(() => {
    ...
    if (!isSanchalak || !selectedCentreId) return all;
    return all.filter((a) => a.centre_id === selectedCentreId);
  }, [list.data?.items, isSanchalak, selectedCentreId]);

  const emptyCreateHint = isSanchalak
    ? hi
      ? "अभी कोई गृहकार्य नहीं। इस केंद्र के बैच के लिए नया असाइनमेंट बनाएँ।"
      : "No homework yet. Create an assignment for a batch at this centre."
    : hi
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.background }}>
      ...
      <Screen refreshing={list.isRefetching} onRefresh={list.refetch}>
        ...
        {list.isLoading || (isSanchalak && centresQ.isLoading) ? (
