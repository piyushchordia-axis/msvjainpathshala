/**
 * Debounced server-side picker search (GST-PRF-03) — mobile mirror of the
 * web `lib/picker-search.ts`.
 *
 * The join pickers load one clamped page of /v1/public/cities|centres and
 * filter it on-device — on a network larger than the clamp, a centre beyond
 * the first page simply could not be chosen. Typing now also queries the
 * server's `?q=` and merges the hits into the local list.
 */
