...
/**
 * Notices screen. A guest sees the public feed (/v1/notices/public). A signed-in
 * member sees the audience-scoped feed (/v1/notices/feed) — national + their
 * state/city/centre/batch + MSV, including internal (non-public) notices — and
 * each notice is marked read on open so unread counts stay accurate.
 */
...
export default function GuestNoticesScreen() {
  ...
  // Public feed for guests; scoped feed for signed-in members.
  ...
  // Mark the visible scoped notices read once they load (best-effort, idempotent).
  useEffect(() => {
